// import React, { useEffect, useState } from "react"; import image from "../assets/stone.png";

// export default function BulkOrderForm() { // -------------------- Helpers (no external libs) -------------------- const nameRegex = /^(?=.{1,100}$)[\p{L}]+(?:[ .'-][\p{L}]+)*$/u; const emailRegex = /^[^\s@]+@[^\s@]+.[^\s@]{2,}$/;

// // fixed: escaped + and optional separator const phoneRegex = /^(?:+91[-\s]?)?[6-9]\d{9}$/; const pinRegex = /^[1-9][0-9]{5}$/; const gstRegex = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/i; const twoDecimalRegex = /^-?\d+(?:.\d{1,2})?$/;

// const MAX_MESSAGE_WORDS = 50; const MAX_MESSAGE_CHARS = 300; const countWords = (str = "") => { const s = String(str).trim(); if (!s) return 0; return s.split(/\s+/).length; };
...
// const validateFile = (file, maxMB = 50) => { if (!file) return null;
...
// const ErrorText = ({ children }) => ( <p className="flex items-center gap-1 text-sm text-red-600 mt-1" role="alert"> <svg
...
// // helper which returns classes: only show red when form was submitted AND field has error const inputClass = (field) => w-full border rounded-2xl px-3 py-3 text-gray-700 outline-none ${ isSubmitted && errors[field] ? "border-red-500 focus:ring-2 focus:ring-red-500" : "border-gray-300 focus:ring-2 focus:ring-pink-700" };
...
// // NOTE: removed live per-field validation on change.
// // We only set isSubmitted on submit and then errors are displayed.
...
// return ( <div className="min-h-screen font-poppins bg-white px-4 sm:px-6 lg:px-16 py-8"> <div className="max-w-5xl mx-auto"> <div className="mb-6"> <h1 className="text-2xl sm:text-4xl font-semibold text-[#000000]">Stonepedia Bulk Orders</h1> <p className="text-sm sm:text-lg text-[#BDBDBD]">Fill this form to connect with us..</p> </div>
...
//                   onChange={handleChange}
//                   className={`${inputClass("companyName")} max-w-[317px]`}
...
//                 onChange={handleChange}
//                 placeholder="Choose"
//                 className={`${inputClass("gstNumber")} mt-[6px]`}
//               />
...
//                   onChange={handleChange}
//                   className={inputClass("productFinish")}
//                 >
...
//               <div className={`flex items-center w-full rounded-2xl px-3 py-3 text-gray-500 ${
//                 isSubmitted && errors.value
//                   ? "border-red-500 focus-within:ring-2 focus-within:ring-red-500"
//                   : "border-gray-300 focus-within:ring-2 focus-within:ring-pink-700"
//               }`}>
//                 <input
//                   type="text"
//                   name="value"
...
//                       onChange={handleChange}
//                       className={`w-full border rounded-2xl px-3 py-3 text-gray-500 outline-none ${
//                         isSubmitted && errors.width
//                           ? "border-red-500 focus:ring-2 focus:ring-red-500"
//                           : "border-gray-300 focus:ring-2 focus:ring-pink-700"
//                       }`}
//                     />
...
//                   onChange={handleChange}
//                   className={`w-full border rounded-2xl px-3 py-3 text-gray-500 outline-none ${
//                     isSubmitted && errors.deliveryTime
//                       ? "border-red-500 focus:ring-2 focus:ring-red-500"
//                       : "border-gray-300 focus:ring-2 focus:ring-pink-700"
//                   }`}
//                 />
...
//             {/* product image upload area (unchanged visuals) */}
...
//           <div className="mb-5 border border-dashed border-[#871B58] rounded-lg p-4 text-center text-gray-500 relative bg-white hover:shadow-md transition">
...
//           {/* Installation */}
//           <div className="h-auto min-h-[160px] border w-full max-w-[688px] border-dashed border-gray-300 rounded-lg p-4 text-gray-500 relative">
//             <div className="mb-4">
...
//                 onChange={handleChange}
//                 className="w-full border border-gray-300 rounded-2xl px-3 py-3 text-gray-500 focus:ring-2 focus:ring-pink-700 outline-none"
...
//             <textarea name="message" className={`${isSubmitted && errors.message ? "border-red-500 focus:ring-2 focus:ring-red-500" : "border-gray-300 focus:ring-2 focus:ring-pink-700"} w-full h-auto min-h-[120px] rounded-2xl px-3 py-3 text-gray-500 outline-none`} placeholder="Write your message here" rows={3} value={formData.message} onChange={handleChange} />
//           </div>
